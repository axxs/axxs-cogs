...
def normalize_name(text: str) -> str:
    """Collapse case, whitespace, dashes, and underscores. Same function is
    applied to user input and to place keys/aliases so the two sides match."""
...
def _sources_from_entry(entry: dict, *, place_key: str, path: Path) -> list:
    ...
    if "eventbrite_slug" in entry:
        slug = str(entry["eventbrite_slug"]).strip()
        return [Source(kind="eventbrite", spec={"slug": slug})]

    raise RegionNotFoundError(
        f"Place '{place_key}' in {path} has neither 'sources' nor 'eventbrite_slug'."
    )
